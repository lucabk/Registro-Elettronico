...
const SingleTeacher = ({ teacher, setTeachers }) => {
    ...
    const handleDelete = async (e) => {{
        e.preventDefault()
        if(window.confirm(`Cancellare il docente ${teacher.cognome} ${teacher.nome}?`)){
            try{
                await docenteService.deleteTeacher(teacher.id)
                setTeachers(prev => prev.filter(d => d.id !== teacher.id))
                toast.success("Docente eliminato correttamente")
            }catch(e){
                toast.error("Impossibile cancellare il docente")
                console.error("Impossibile cancellare il docente: ", e)
            }
        }
    }}
