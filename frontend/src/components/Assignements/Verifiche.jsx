...
const Verifiche = ({ teacher, incarichi, classeId}) => {
    ...
    const handleDelete = (v) => async (e) => {
        e.preventDefault()
        if(v.incaricoDTO.docenteDTO.id === teacher.id && window.confirm("Cancellare verifica?")){
            try{
                await examService.deleteVerifica(v.id)
                setVerifiche(prev => prev.filter(vr => vr.id !== v.id))
                toast.success("Verifica eliminata correttamente")
            }catch(e){
                toast.error("Impossibile cancellare verifica")
                console.error("Impossibile cancellare verifica: ", e)
            }
        }
    }
                            ...
                            <ul>
                                {verficaDelGiorno.map(v => (
                                    <li key={v.id}>
                                        ...
                                        {teacher?.id === v.incaricoDTO.docenteDTO.id  && (
                                            <button type="button" onClick={handleDelete(v)} className="btn">
                                                ...
                                                <span className="visually-hidden">Delete exam</span>
                                            </button>
                                            )}
                                    </li>
                                ))}
